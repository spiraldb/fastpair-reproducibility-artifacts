# `a100-fsst12` — FSST-12 decoded by the shipped FastPair kernels

A100-SXM4 40GB (sm_80), GCP us-central1-f, vortex rev `2d909147f` (branch `mp/fsst12-bench`).
Two harness runs, both at that rev, contribute to this directory:

- `a100-fsst12-timing-20260813-011737` (2026-08-13) — the five HuggingFace text columns.
- `a100-fsst12-recover-20260813-234855` (2026-08-13) — ClickBench, TPC-H, synthetic URL.
  These were never collected by the first run, whose launcher named its result files
  explicitly and carried only the HF columns; the recover launcher globs instead. Two earlier
  zones (`us-central1-a`, `us-central1-c`) returned STOCKOUT and launched nothing.

Both runs are **timing only, no NCU capture**, deliberately. An earlier A100 attempt carried
the capture and the GPU hardware-faulted mid-run (`nvidia-smi` reporting `ERR!`, every
subsequent launch returning CUDA error 9); the capture-identity assertion in
`jobs/onpair-shdict-ncu.sh` marked all five post-fault captures CAPTURE-INVALID, so no bad
data entered. The width finding already spans B300, H100 and L40S captures, so an A100
capture is redundant rather than load-bearing. Capturing it remains an open follow-up.
