# OnPair CPU decode sweep — 4 physical cores (full)
...
at all costs. Metric: GiB/s of decoded output, at **1/2/4 threads**, **256 MiB/column**.
...
Every box `*.2xlarge`, pinned to **4 physical cores** (one logical CPU per core via `lscpu -e`, so
thread counts map to real cores not SMT siblings). Uniform `.2xlarge` memory slice across all 10
machines → apples-to-apples regardless of SMT. 10 machines: AMD Rome/Milan/Genoa/Turin, Intel
...
`python3 results/cpu-sweep/combine.py results/cpu-sweep-4phys`. Real columns are produced locally by
