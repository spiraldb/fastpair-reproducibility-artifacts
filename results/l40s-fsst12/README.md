...
L40S (sm_89, GDDR6), Nebius eu-north1, vortex rev `2d909147f` (branch `mp/fsst12-bench`).
Two harness runs, both at that rev, contribute to this directory:

- `l40s-fsst12-20260812-225451` (2026-08-12) — the five HuggingFace text columns.
- `l40s-fsst12-recover-20260813-235420` (2026-08-13) — ClickBench, TPC-H, synthetic URL.
  ...
  box. The recover launcher globs instead. Timing only, no NCU capture. An earlier recovery
  attempt (`l40s-fsst12-recover-20260813-234854`) was preempted just after its build phase and
  produced no cells; teardown confirmed, nothing from it is here.
...
- **The two bases agree only where cardinality is high.** They are equal to two decimals on
  ...
  name the column set it covers. The ratios are properties of the codec and the container, not
  of the chip, so they are identical across the four `*-fsst12` directories.
